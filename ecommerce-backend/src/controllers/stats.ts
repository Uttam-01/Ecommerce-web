...
import { calculatePercentage, getChartData, getInvantory } from "../utils/features.js";
...
export const getDashboardStats = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key)) stats = JSON.parse(myCache.get(key) as string)
        ...
        const categoryCount: Record<string, number>[] = await getInvantory({ categories, productsCount });
...
export const getPieChart = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key)) charts = JSON.parse(myCache.get(key) as string);
        ...
        const productCategories: Record<string, number>[] = await getInvantory({ categories, productsCount });
...
export const getBarChart = TryCatch(async (req, res, next) => {
    ...
    if (myCache.has(key)) charts = JSON.parse(myCache.get(key) as string);
        ...
        const sixMonthsUsersPomise = User.find({
            createdAt: {
                $gte: sixMonthsAgo,
                $lte: today,
            }
        ...
        const tweleveMonthsOrdersPromise = Order.find({
            createdAt: {
                $gte: twelveMonthsAgo,
                $lte: today,
            }
        ...
        const [
            ...
            sixMonthsUsersPomise,
            tweleveMonthsOrdersPromise
        ]);
